...
param(
    ...
    [Parameter(Mandatory = $true)][string]$BrowserStoreUrl,
    [ValidateSet('Firefox', 'Edge')][string]$BrowserName = 'Firefox',
...
if ($BrowserStoreUrl -notmatch '^https://[^\s]+$') { throw 'BrowserStoreUrl must be an HTTPS URL.' }
...
$signatureLabel = if ($signature.Status -eq 'Valid') {
    "Authenticode signature valid: $($signature.SignerCertificate.Subject)"
}
else {
    'Unsigned preview - not for public distribution'
}
$sourcePublicName = if ($FfmpegSourceArchive -and $FfmpegSourceArchive.EndsWith('.tar.zst', [StringComparison]::OrdinalIgnoreCase)) {
    'ffmpeg-corresponding-source.tar.zst'
}
else {
...
$productActions = @"
<a class="button" href="$BrowserStoreUrl">Get the $BrowserName extension</a><a class="button secondary" href="download.html">Install the Windows Helper</a>
"@
$helperDownloadBlock = @"
<div class="notice"><p>Version $version · Windows 10 or later · $signatureLabel</p></div>
...
"@
$ffmpegSourceBlock = @"
<section><h2>FFmpeg corresponding source</h2><p>The exact corresponding-source package for the FFmpeg binaries distributed with Helper $version is available here:</p><p><a href="source/$sourcePublicName">Download FFmpeg corresponding source</a></p><p>Build identification and checksums are included with the archive. Questions may be sent to <a href="mailto:$SupportEmail">$SupportEmail</a>.</p></section>
"@
...
if ($FfmpegSourceArchive -and (Test-Path -LiteralPath $FfmpegSourceArchive -PathType Leaf)) {
    Copy-Item -LiteralPath $FfmpegSourceArchive -Destination (Join-Path $sourceRoot $sourcePublicName) -Force
}
